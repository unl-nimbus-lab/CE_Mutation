...
base_value= rt_altitude;
base_sti = stepinfo(rt_altitude,rt_t,62.0);
...
%             step_charac_PeakTime(i)= -1000;
        ...
        load(file)
        sti = stepinfo(rt_altitude,rt_t,62.0);
...
% Create figure
figure1 = figure('Name','Boeing 747 altitude % Live mutants');
...
% Create figure
figure2 = figure('Name','Boeing 747 altitude % Dead mutants');
...
%         load(file)
%         sti = stepinfo(rt_altitude,rt_t,151.148);
%         
%         %increase mutant count
%         running_mutant_count = running_mutant_count + 1;
%         
%         is_blow_up = false;
%         mutant_value = rt_altitude;
%         error = 0;
%         model = 0;
%         max_error = 0;
%         
...
% load('/home/balaji/clang-llvm/cppdocker/code/mutator/base_original.mat')
% base_value= rt_altitude;
% sti = stepinfo(rt_altitude,rt_t,150.148);
% 
% x=1:1:nooffiles;
% %plot(x,step_charac_RiseTime,x,step_charac_SettlingTime,x,step_charac_SettlingMin,x,step_charac_SettlingMax,x,step_charac_Overshoot,x,step_charac_Undershoot,x,step_charac_Peak,x,step_charac_PeakTime);
% 
